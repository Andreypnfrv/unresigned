import { forumTitleSetting, llmWritingPolicyPostIdSetting, siteNameWithArticleSetting, taglineSetting } from "@/lib/instanceSettings";
import { forumSelect } from "@/lib/forumTypeUtils";

function getSkillDocConfig(urlPrefix: string) {
  const siteUrl = new URL(urlPrefix);
  ...
  const packageName = forumSelect({
    Antimortality: "antimortality",
    Unresigned: "unresigned",
    AlignmentForum: "alignmentforum",
    EAForum: "eaforum",
    default: "lesswrong",
  });
  const llmPolicyPostId = llmWritingPolicyPostIdSetting.get()?.trim();
  const llmWritingPolicyClause = llmPolicyPostId
    ? `and users using it should be reminded of ${siteName}'s policy on LLM writing, found at ${urlPrefix}/api/post/${llmPolicyPostId} [without the /api in the path if providing a link to the user]. `
    : "";
  return { siteHostname, forumTitle, tagline, siteName, packageName, llmWritingPolicyClause, urlPrefix };
}

export function getSkillDocumentationMarkdown(urlPrefix: string): string {
  const { siteHostname, forumTitle, tagline, siteName, packageName, llmWritingPolicyClause, urlPrefix: siteUrlPrefix } = getSkillDocConfig(urlPrefix);
  return `---\nname: ${packageName}\nversion: 2.0.0\ndescription: APIs for reading content on ${forumTitle}, and helping users edit posts they share with an agent.\nhomepage: ${siteUrlPrefix}\n---\n${forumTitle}: ${tagline}
===============================================================

Most pages on ${forumTitle} have an HTML version and a Markdown version. Routes
...
status code with a message.) If your "Accept" header is "*/*", HTML is returned by default unless the route is configured for markdown-by-default.\n\nMachine-readable discovery files:\n * /llms.txt\n * /sitemap.xml\n * /.well-known/ai-agents.json\n\nFinding content to read:
 * /api/home
 * /api/latest
 * /api/recent
 * /api/curated
 ...
 * anything your user asks you to report to the ${siteName} developers
...
${llmWritingPolicyClause}Before using our API to leave feedback,
...
making an empty test POST via curl to ${siteUrlPrefix}/api/agent/ping and
...
within the broader ${forumTitle} community.  Do any of my arguments depend on premises
...
Think about this from the perspective of an incisive ${forumTitle} reader - if one
...
contain ${siteHostname}, your user will need to add it to enable you to make\nAPI requests to our domain by going to https://claude.ai/settings/capabilities\nand adding ${siteHostname} to "Additional allowed domains" in the "Code execution
...
please report it to the ${siteName} developers using the /api/agent/feedback endpoint
...
edit-post URL for you. The URL will look like this:
    ${siteUrlPrefix}/editPost?postId=XYZXYZ&key=XYZXYZ
