...
export const generateLoadingMessagePrompt = (query: string, postTitle?: string): string => {
  return [
    'I need you to generate some humorous "loading messages" to display to users of the Unresigned.com Claude chat integration since it can take 10-30 seconds to load new responses',
    'Your responses may make general humorous references to Unresigned, but it is even better if they are tailored to the specific query or post the user is viewing.',
...
export const CONTEXT_SELECTION_SYSTEM_PROMPT = [
  'You are part of a system interfacing with a user via chat window on Unresigned.com.',
  'Your responsibility is to make decisions about whether or not to load Unresigned posts as additional context for responding to user queries, and what strategy to to employ.'
].join('\n');

export const contextSelectionChoiceDescriptions = `(0) none - No further context seems necessary to respond to the user's query because Claude already has the knowledge to respond appropriately, e.g. the user asked "What is Jacobian of a matrix?" or "Proofread the following text." Alternatively, the answer might be (0) "none" because it seems unlikely for there to be relevant context for responding to the query in the Unresigned corpus of posts.

(1) query-based - Load Unresigned posts based on their vector similarity to the user's query, and ignore the post the user is interacting with. This is the correct choice if the query seems unrelated to the post the user is interacting with, but it seems likely that there are Unresigned posts concerning the topic. (If it is a very general question, the correct choice might be (0) "none").

(2) current-post-only - Load the current Unresigned post into context but nothing else. This is the correct choice if the query seems to be about the post the user is currently interacting with and further context is unnecessary. For example, if the user asks for a summary or explanation of the current post.

(3) current-post-and-search - Load the posts the user is currently interacting with and similar posts based on vector similarity to the post the user is interacting with (but NOT posts based on vector similarity to the query). This is the correct choice if the query seems to be about the post the user is currently viewing, and pulling up other Unresigned posts related to the current post is likely to be relevant but a search based on the user's query would either be redundant with a search based on the current post, or would return irrelevant results. Some examples of such queries that should get current-post-and-search are: "What are some disagreements with the arguments in this post?", "Explain <topic in the post> to me."

(4) both - Load Unresigned posts based on their vector similarity to both the user's query and the post the user is interacting with. This is the correct choice if the question seems to be related to the post the user is currently interacting with, but also contains keywords or information where relevant Unresigned posts would be beneficial context for a response, and those Unresigned posts would not likely be returned in a vector similarity search based on the post the user is currently interacting with. If the question does not contain technical terms or "contentful nouns", then do not select "both", just select one of "current-post-only" or "current-post-and-search".`;
...
export const generateContextSelectionPrompt = ({ query, postContext, currentPost }: BasePromptArgs): string => {
  ...
  return `The user has sent a query: "${query}".  ${currentPostContextLine}  Based on the query${currentPostContextClause}, you must choose whether to load additional Unresigned posts as context, and if so, based on what criteria. Remember, your options are:
...
export const CLAUDE_CHAT_SYSTEM_PROMPT = [
  `You are an expert research assistant providing assistance to students and researchers on Unresigned.com.  You are highly knowledgeable about both technical and philosophical topics, including advanced math, physics, and computer science.`,
  `Users interact with you via a chat window that is available on all pages on Unresigned.  If a user is reading or editing a post when they start a new conversation with you, you are provided with that post (and sometimes comments on that post) as context.  Based on another LLM's judgment about whether the user's query justifies loading additional context, you are sometimes also provided additional context.  If present, that context will be additional Unresigned posts, which are determined by an embedding similarity search on either the post the user is interacting with (if any), the user's query, or the combined results of both searches.  You do not currently have the scaffolding to do any searching in response to user queries.  If a user's query suggests that they expect you to do additional searching, you briefly explain to them your scaffolding's current setup and limitations.`,
...
${currentPostLine}${additionalPostsBlock}${currentPostContentBlock}${commentsOnPostBlock}${providedPostsBlock}
  ...
  return `<AdditionalInstructions>You are now interacting with a user via chat window on Unresigned.com.  ${userInfo}  The user has sent the following query: "${query}".
${contextBlock}
